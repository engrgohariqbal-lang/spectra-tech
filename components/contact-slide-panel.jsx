...
export function ContactSlidePanel() {
  ...
  return (
    <div
      className={cn(
        "fixed right-0 top-1/2 -translate-y-1/2 z-50 flex shadow-lg transition-transform duration-300 ease-in-out",
    ...
    >
      {/* Sliding Content */}
      <div className="bg-white border border-border border-r-0 rounded-l-lg p-6 flex flex-col gap-6 shadow-xl w-72">
        <h3 className="font-bold text-primary text-lg border-b pb-2">
          Contact Us
        </h3>
      ...
      <button
        ...
        className={cn(
          "absolute left-0 top-1/2 -translate-y-1/2 -translate-x-full bg-primary text-white rounded-l-lg shadow-md cursor-pointer flex items-center justify-center p-2 transition-all hover:bg-primary/90",
          "w-12 h-32 overflow-hidden",
        )}
        aria-label="Toggle Contact Panel"
      >
        <span className="-rotate-90 font-bold tracking-[0.2em] text-sm whitespace-nowrap">
          CONTACT
        </span>
      </button>
    </div>
